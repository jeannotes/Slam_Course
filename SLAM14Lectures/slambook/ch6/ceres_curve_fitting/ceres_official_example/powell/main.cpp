...
int main(int argc, char **argv){
    ...
    double x1 = 3.0, x2 = -1, x3 = 0, x4 =1;

    Problem problem;
    problem.AddResidualBlock(new AutoDiffCostFunction<F1, 1, 1, 1>(new F1), NULL, &x1, &x2);
    /**
     * in AutoDiffCostFunction first residual dimension, then x-dim, y-dim
     * in AddResidualBlock, first is CostFunction, then lostfunction, after that
     * just variable, x, y, z.....
     */
    ...
    std::cout<<x1<<std::endl<<x2<<std::endl<<x3<<std::endl<<x4;
    return 0;
}